...
class IG_MSA(nn.Module):
    ...
    def forward(self, x_in, illu_fea_trans):
        ...
        attn = attn.softmax(dim=-1) #b,heads,c,c
...
if __name__ == '__main__':
    ...
    # dim_head = 64
    # heads = 8
    # x_in = torch.rand(8, 224, 224, 31)
    # b, h, w, c = x_in.shape
    # x = x_in.reshape(b, h * w, c)
    # to_q = nn.Linear(dim, dim_head * heads, bias=False)
    # q_inp = to_q(x)
    # k_inp = to_q(x)
    # v_inp = to_q(x)
    
    # illu_attn = torch.rand(8, 3, 224, 224) # illu_fea: b,c,h,w -> b,h,w,c
    # # print(illu_attn.shape)
    # q, k, v, illu_attn = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=heads),
    #                             (q_inp, k_inp, v_inp, illu_attn.flatten(1, 2)))
    # print(q.shape)
    # q = q.transpose(-2, -1)
    # print(q.shape)
