...
class SwinEnhancer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=1, in_chans=3,
                 ...
                 **kwargs):
        super(SwinEnhancer, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        ...
        self.reconstruction_layer = nn.Sequential(
            ...
            nn.Conv2d(in_channels=embed_dim, out_channels=num_feat, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=num_feat, out_channels=num_out_ch, kernel_size=3, padding=1),
        )
        
        self.activation = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        # self.final_activation = nn.Sigmoid() if img_range == 1. else nn.Tanh()
    ...
    def forward(self, x):
        ...
        #activation
        x = self.activation(x)
        x = self.final_activation(x) + orig_x
        # x = x + self.final_activation(self.activation(x))
...
class SwinIR(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_after_body(self.forward_features(x)) + x
            x = self.conv_before_upsample(x)
            # x = self.lrelu(self.conv_Pup1(torch.nn.functional.interpolate(x, scale_factor=2, mode='nearest')))
