...
def test_real(model, dataloader, device, save_dir):
    ...
    with torch.no_grad():  # Disable gradient computation during testing
        for i, low_light_imgs in enumerate(dataloader):
            ...
            # low_output = model(low_light_imgs)
            I_low1, I_map, I_low, enhanced_I_low1, enhanced_I_low, reconstruct1, reconstruct = model(low_light_imgs)
            # I_low_3 = torch.concat([I_low, I_low, I_low], dim=1)
            # low_output = R_low*I_low_3
            save_path1 = os.path.join(save_dir, f"test_batch_{i+1}_I_low1.jpg")
            save_path2 = os.path.join(save_dir, f"test_batch_{i+1}_I_map.jpg")
            save_path3 = os.path.join(save_dir, f"test_batch_{i+1}_I_low.jpg")
            save_path4 = os.path.join(save_dir, f"test_batch_{i+1}_enhanced_I_low1.jpg")
            save_path5 = os.path.join(save_dir, f"test_batch_{i+1}_enhanced_I_low.jpg")
            save_path6 = os.path.join(save_dir, f"test_batch_{i+1}_reconstruct1.jpg")
            save_path7 = os.path.join(save_dir, f"test_batch_{i+1}_final.jpg")
            # save_path1 = os.path.join(save_dir, f"test_batch_{i+1}_truth.jpg")
            save_path8 = os.path.join(save_dir, f"test_batch_{i+1}_low_original.jpg")
            save_image(I_low1, save_path1, normalize=True)
            save_image(I_map, save_path2, normalize=True)
            save_image(I_low, save_path3, normalize=True)
            save_image(enhanced_I_low1, save_path4, normalize=True)
            save_image(enhanced_I_low, save_path5, normalize=True)
            save_image(reconstruct1, save_path6, normalize=True)
            save_image(reconstruct, save_path7, normalize=True)
            # save_image(well_lit_imgs, save_path1, normalize=True)
            save_image(low_light_imgs, save_path8, normalize=True)
...
if __name__ == "__main__":
    ...
    save_dir = "./train_prune/LOLv2Syn_prune_Dsize"
    ...
    #700->24.4/0.91
    #699->23.65
    save_dir = "./Test_image/LOLv2Syn_prune_Dsize_sep"
    # best_weights(model, weights_folder="weights/train_prune/LOLv2Syn_prune_Dsize", device=device, test_dataloader=test_dataloader, save_dir=save_dir)
    state_dict = torch.load("./weights/train_prune/LOLv2Syn_prune_Dsize/model_epoch_699.pth")
    ...
    test_dataset = UnpairedLowLightLoader("Train_data/sep")
    test_dataloader = DataLoader(dataset=test_dataset, batch_size=1, shuffle=False)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
