...
def train(model, train_loader, val_dataloader, device, save_dir, num_epochs=150):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f"./weights/LOLv1_quan.pth")
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('./train_quan/LOLv1_quan/training_validation_loss_plot.png')
...
if __name__ == "__main__":
    model = RetinexUnet()
    save_dir = "./train_quan/LOLv1_quan"
    ...
    #------------------perform pruning------------------#
    saved_model_path = "./weights/LOLv1_quan.pth"
    ...
    save_dir_quan = "./train_quan/LOLv1_quan"
    ...
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    state_dict = torch.load("./weights/LOLv1_quan.pth")

    # Create a new state dictionary with the "module." prefix removed from each key
    new_state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
    model.load_state_dict(new_state_dict)  # Load the trained weights
    model.to(device)
    save_dir = "./Test_image/LOLv1_quan"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
